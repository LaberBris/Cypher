...
//请补充代码
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++) 
        {
            array[i][j] = GFMul(deColM[i][0], temp[0][j]) ^ GFMul(deColM[i][1], temp[1][j]) 
...
//请补充代码
    ...
    {
        int col[4];
        splitIntToArray(w[round * 4 + j], col);
    ...
    for(int i = 0; i < 4; i++)
        for(int j = 0; j < 4; j++)
            array[i][j] = array[i][j] ^ k_round[i][j];
}
...
//请补充代码
    ...
    splitIntToArray(num, row);
    // 循环左移
    leftLoop4int(row, 1);
    // 字节代换
    for (int i = 0; i < 4; i++)
        row[i] = getNumFromSBox(row[i]);

    // 与轮常数异或
    result = mergeArrayToInt(row);
    result ^= Rcon[round];
